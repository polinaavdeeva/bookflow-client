...
const AddBookPopup: FC<IAddBookProps> = ({
  ...
  useEffect(() => {
    BookServices.bookSearch(" ").then((resp) => {
      const book = resp.find(
        (book: { name: string }) =>
          book.name.toLowerCase() === name.toLowerCase()
      );
      if (book) {
        setBookId(book._id);
        setExsistingBookName(book.name);
      } else {
      }
    });
  }, [name]);
  ...
  const postingDate = `${currentDate.getDate().toString().padStart(2, "0")}.${(
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const result = await response.json().then((resp) => {
        BookServices.uploadBookImage(image, resp._id)
          .then((data: any) => {
  ...
  return (
    <section className={`popup ${isOpen ? "popup_opened" : ""}`}>
      ...
      <section
        className={`popup ${isOpenWarningMessage ? "popup_opened" : ""}`}
      >
        <div className="popup__container" style={{ width: 600, height: 200 }}>
          ...
          <div style={{ textAlign: "center", paddingBottom: 3 }}>
            Информация о книге с таким названием уже была добавлена ранее.
          </div>
          <div style={{ textAlign: "center", paddingBottom: 30 }}>
            Ваша книга будет добавлена на соответсвующую страницу.
          </div>
          <Button
            className="popup__add-button"
            label="Хорошо"
